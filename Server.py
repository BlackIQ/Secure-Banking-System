...
class Server:
    ...
    def start_server(self):
        ...
        while True:
            ...
            if self.state == -1:
                ...
                msg = self.c1.recv(4096).decode(errors='ignore')
    ...
    def receive_message_state_negative1(self, msg):
        # secure_key_parts = msg.split(maxsplit=1)
        # print(secure_key_parts)
        # secure_signed_key = Encryption_Decryption.decryption_server_private_key(secure_key_parts[0].encode())
        secure_key = Encryption_Decryption.decryption_server_private_key(msg)
        print(secure_key)
        # secure_key_digest = SHA256.new(secure_key)
        # secure_key_verification = Sign_Verify.verify_client_public_key(secure_signed_key, secure_key_digest)
        # if secure_key_verification == 1:
...
server = Server(Login(MysqlConnection()), Signup(MysqlConnection()), MysqlConnection(),
                BankingOperation(MysqlConnection()))
